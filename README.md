# F1 Fantasy Team Optimization: A Contexual Linear Bandit Approach
...
- Human baseline: 8,485 points
- Standard Bandit: 8,585 points
- Greedy baseline: 6,272 points
...
1. **Load and explore data:**
   - Run `fantasyf1.ipynb` to load race data, driver stats, and track characteristics
   - Features are automatically engineered from raw CSV files
...
2. **Train the bandit:**
   ...
   # Train on historical data (races 1-70)
   for race in training_races:
       team = select_team(bandit, race)
       points = observe_results(team, race)
       bandit.update(points)
   ```

3. **Evaluate:**
   - Test on 2024-2025 races (48 races)
   - Compare Enhanced vs Standard Bandit
   - Analyze feature importance

## Files

**Notebook:**
- `fantasyf1.ipynb` - Complete implementation with data loading, feature engineering, algorithm, and evaluation

**Report:**
- `report.pdf` - Detailed IEEE-format technical report

**Data (2023-2025):**
- `Fantasy_F1_Driver_Race_Data_*.csv` - Per-race driver points and costs
- `Fantasy_F1_Team_Race_Data_*.csv` - Per-race constructor points and costs
- `Fantasy_F1_Track_Race_Data_*.csv` - Track characteristics (length, turns, DRS zones, weather, type)
- `Fantasy_F1_Driver_Stats_Data_*.csv` - Driver career stats (podiums, championships, experience, age)
- `Fantasy_F1_My_Data_2024_2025.csv` - Human baseline selections and points (author's picks)
...
```
Sullivan Montgomery. F1 Fantasy Team Optimization: A Contexual Linear Bandit Approach. 2026.
```
